...
- **Before selecting tickets:** Read this file if it exists. Build a set of ignored page IDs. When building the candidate list (after filter and sort), exclude any page whose `id` is in that set. **Also exclude** any ticket that already has a corresponding plan in `~/.cursor/plans/` (see below) **or** any saved investigation in `~/.cursor/saved-investigations/` (see below). Then take the first **N** remaining.
...
- **Exclude tickets that already have a saved investigation:** List files in `~/.cursor/saved-investigations/` (`.md` only). A ticket is considered to have a saved investigation if any file name starts with `{short_id}-` (e.g. `ABCDE-2025-02-26T143052.md`). Exclude those tickets from the list before taking the top N.
...
**Get top N and apply ignore list and existing plans:** Run the parser so you get structured lines you can use (id, url, title, short_id). Parser path: `python3 /home/romain/Stockly/.cursor/parse_qtt.py <path_to_json_file> [N]` — N defaults to 3. It reads the JSON from the Notion query result file, filters (Status Intl = Pending workforce, Assignee empty, Teams Intl not Partner Inputs_Front), sorts by Severity then Updated At, prints `candidates_count=<m>` on stderr and one line per ticket `rank|id|url|title|short_id` for the top N. Then exclude any line whose `id` is in the ignore set, any ticket whose short_id already has a corresponding plan in `~/.cursor/plans/` (filename `<short_id>-*.md` or plan content with `## Short ID` equal to that short_id), **and** any ticket whose short_id already has at least one file in `~/.cursor/saved-investigations/` (filename starting with `{short_id}-`). Take the first **N** tickets from the remaining list (if the parser was run with a large N, you already have a sorted list; drop ignored and already-planned, then take first N).
...
**After producing and displaying the resume for a ticket:** Persist the investigation result so the user can review it later (e.g. after closing the window or in a new agent). (1) Ensure `~/.cursor/saved-investigations/` exists (create the directory if it does not). (2) Write one `.md` file there with filename `{short_id}-{date}T{time}.md` (e.g. `ABCDE-2025-02-26T143052.md`; use ISO date and time without colons). File content: a header with `# {ticket title}`, `Short ID: {short_id}`, `Notion: {notion_url}`, `Notion page ID: {id}` (use the parser result `id` field), `Investigation date: {iso-date}`; then `## Resume` and the super-synthetic resume block you just displayed; then `## Full investigation` and the full subagent response. This allows the review command to show summaries and clean up Done tickets.
...
- Remind the user they can run **/review-saved-investigations** to list past investigation results (grouped by short_id, with copy-pastable commands to open the full files) and to clean up saved investigations for tickets whose Status Intl is done.
