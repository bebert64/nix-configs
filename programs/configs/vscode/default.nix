...
let
  ...
  open-local = "${pkgs.writeScriptBin "open-local" ''
    selection=$(
      ...
      sort -u | \
      rofi -i -dmenu -no-custom -theme-str 'window {width: 20%;}' -p ""
    )
  ...
  open-cerberus = "${pkgs.writeScriptBin "open-cerberus" ''
    selection=$(
      ...
      sort -u | \
      rofi -i -dmenu -no-custom -p "" -theme-str 'window {width: 30%;}'
    )
  ...
  open-salon = "${pkgs.writeScriptBin "open-salon" ''
    selection=$(
      ...
      sort -u | \
      rofi -i -dmenu -no-custom -p "" -theme-str 'window {width: 20%;}'
    )
    if [[ $selection = "code" ]]; then
