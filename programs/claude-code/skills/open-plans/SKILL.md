...
3. List all `.md` files in `~/.claude/plans` (or the user's Cursor plans directory) whose **filename starts with `<short_id>-`**. That includes:
...
6. If no file in `~/.claude/plans/` has a name starting with that short_id, say so (e.g. "No plans or investigations found for short_id XYZ.").
