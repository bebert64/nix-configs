...
|---|---|
| A coding convention, style preference, or constraint | **Rule** (`.md` in `rules/`) |
...
Rules live in `~/.claude/rules/` (global or stockly subdirectory). Source of truth: `nix-configs/programs/claude-code/rules/`.
...
Skills live in `~/.claude/skills/<skill-name>/SKILL.md`. Source of truth: `nix-configs/programs/claude-code/skills/`.
...
- **Global** (`rules/global/`, `skills/global/`, `docs/global/`): for rules/skills/docs useful across all projects.
- **Stockly-specific** (`rules/stockly/`, `skills/stockly/`, `docs/stockly/`): for artifacts tied to Stockly codebase, tooling, or conventions.
- **Perso** (`skills/perso/`): for personal project skills.
...
All artifacts are managed in **nix-configs** (`programs/claude-code/`) and symlinked to `~/.claude/` by home-manager. Edit the nix-configs source, not the symlinks.
